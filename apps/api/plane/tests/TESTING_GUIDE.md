...
```python
import pytest
from plane.app.serializers import MySerializer
...
Contract tests should be placed in `tests/contract/app/` and should test application API endpoints.
...
2. `session_client`: API client with session authentication (for web app API tests)
3. `create_user`: Creates and returns a test user
4. `mock_redis`: Mocks Redis interactions
