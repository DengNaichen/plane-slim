...
| Non-Django deps (boto3, lxml, etc.)                                                            | —           | **Out of scope** for this PR — keep the change focused on the Django upgrade.                                                                                                                                                                                   |
...
2. **Edit `requirements/base.txt`** per §5.1 + §5.2 (Django, DRF, channels, cors-headers, filter, storages, redis, celery-beat, celery-results, drf-spectacular, psycopg ×3, whitenoise, celery, dj-database-url). Leave django-crum and pytz untouched.
...
12. **Manual smoke** of each entrypoint against a local stack (`docker-compose-local.yml`): **api** (boot + a few endpoints incl. one S3/file-upload path), **worker** (enqueue+run a task), **beat** (scheduler starts, periodic task fires), **migrator** (runs to completion). Confirm `check --deploy` is clean: `python manage.py check --deploy`.
